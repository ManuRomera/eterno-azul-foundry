...
assert.equal(manifest.id, "eterno-azul");
assert.equal(manifest.version, pkg.version);
assert.equal(
  manifest.download,
  `https://github.com/ManuRomera/eterno-azul-foundry/releases/download/v${manifest.version}/eterno-azul.zip`,
);
const tag = process.env.RELEASE_TAG;
if (tag) assert.equal(tag, `v${manifest.version}`);
...
for (const p of await walk("module"))
  if (p.endsWith(".mjs")) execFileSync(process.execPath, ["--check", p]);
for (const p of await walk("templates"))
  Handlebars.precompile(await fs.readFile(p, "utf8"));
